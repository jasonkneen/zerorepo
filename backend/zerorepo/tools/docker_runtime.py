...
class DockerTestRunner:
    ...
    async def _run_tests_docker(self, test_file_path: str, timeout: int) -> Dict:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                self._prepare_test_environment(test_file_path, temp_dir)
                
                # Create container
                container = self.client.containers.run(
                    self.base_image,
                    command=f"python -m pytest {os.path.basename(test_file_path)} -v --tb=short",
                    volumes={temp_dir: {'bind': '/test_env', 'mode': 'rw'}},
    ...
    async def _run_tests_subprocess(self, test_file_path: str, timeout: int) -> Dict:
        ...
        try:
            ...
            cmd = ["python", "-m", "pytest", test_file_path, "-v", "--tb=short"]
    ...
    async def _run_all_tests_docker(self, project_dir: str, timeout: int) -> Dict:
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                container = self.client.containers.run(
                    self.base_image,
                    command=f"bash -c '{install_cmd}python -m pytest tests/ -v --tb=short --json-report --json-report-file=test_results.json'",
    ...
    def _prepare_test_environment(self, test_file_path: str, temp_dir: str) -> None:
        """Prepare isolated test environment."""
        
        import shutil
        
        # Copy test file
        shutil.copy2(test_file_path, temp_dir)
...
"""
        ...
        with open(os.path.join(temp_dir, "setup.sh"), "w") as f:
